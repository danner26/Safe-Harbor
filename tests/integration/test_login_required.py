...
def test_unauthenticated_root_redirects_to_login(client: FlaskClient, configured_user) -> None:
    resp = client.get("/", follow_redirects=False)
    assert resp.status_code == 302
    assert "/login" in resp.location


def test_unauthenticated_root_next_query_preserves_target(
    client: FlaskClient, configured_user
) -> None:
    resp = client.get("/", follow_redirects=False)
...
def test_authenticated_root_returns_200(client: FlaskClient, db_session) -> None:
