"""Animal image upload integration tests."""
...
def _login(client: Any, db_session: Any) -> Any:
    ...
    user = User(email="animal-image-uploader@x.com", password_hash=hash_password("test-pw-12345"))
    db_session.add(user)
    db_session.commit()
...
def _upload_data(filename: str = "clownfish.png", **kwargs: Any) -> dict[str, Any]:
    return {"image": (BytesIO(_image_bytes(**kwargs)), filename)}


def test_unauthenticated_redirects_to_login(client: Any) -> None:
    resp = client.post(f"/animals/{uuid4()}/image", follow_redirects=False)
...
def test_edit_form_renders_image_controls(client: Any, db_session: Any) -> None:
    ...
    animal = _seed_animal(db_session, name="Mabel", image_path="animals/mabel.jpg")

    resp = client.get(f"/animals/{animal.id}/edit")

    assert resp.status_code == 200
    assert b'enctype="multipart/form-data"' in resp.data
    assert bytes(f'action="/animals/{animal.id}/image"', "utf-8") in resp.data
    assert bytes(f'action="/animals/{animal.id}/image/remove"', "utf-8") in resp.data
    assert bytes(f'src="/animals/{animal.id}/image"', "utf-8") in resp.data
...
def test_upload_sets_image_path_and_redirects_to_edit(
    ...
    assert animal.image_path == f"animals/{animal.id}.jpg"
    assert (upload_dir / "animals" / f"{animal.id}.jpg").is_file()
...
def test_rejects_disallowed_extension(
    ...
    assert not (upload_dir / "animals" / f"{animal.id}.jpg").exists()
...
def test_corrupt_file_flashes_and_redirects(
    ...
    resp = client.post(
        f"/animals/{animal.id}/image",
        data={"image": (BytesIO(b"not an image"), "clownfish.jpg")},
        content_type="multipart/form-data",
        follow_redirects=False,
    )
...
def test_replace_overwrites_at_same_path(
    ...
    saved_path = upload_dir / "animals" / f"{animal.id}.jpg"
